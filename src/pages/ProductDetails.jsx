...
const ProductDetails = () => {
  ...
  return (
    <div className='relative max-w-7xl w-full mx-auto my-12 px-6 py-12 border shadow-lg rounded-xl'>
      <Link to={'/'} className='absolute -top-8 -left-1 flex items-center gap-2'>
        <ArrowLeft /> Go back to Home
      </Link>
      <div className='grid grid-cols-1 md:grid-cols-2 gap-6 '>
        <div className='flex justify-center items-center'>
          <img src={product.thumbnail} alt={product.title} className="w-full max-w-md h-[450px] object-contain rounded-xl bg-gray-100 p-8" />
        </div>
            ...
            <div className='flex flex-col md:flex-row justify-between gap-2'>
              <div className='flex gap-2 justify-between items-center border overflow-hidden'>
                ...
                <button
                  ...
                  className={`h-full p-3 ${count===product.stock}?"cursor-not-allowed opacity-50":"hover:bg-gray-100 cursor-pointer "`}
                >
                  <Plus size={18} />
                </button>
              </div>
